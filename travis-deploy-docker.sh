...
cd FROST-Server.HTTP && docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v7,linux/arm/v6 -t adriancierpka/frost-server-http:latest && cd ..
cd FROST-Server.MQTT && docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v7,linux/arm/v6 -t adriancierpka/frost-server-mqttp:latest && cd ..
cd FROST-Server.MQTTP && docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v7,linux/arm/v6 -t adriancierpka/frost-server:latest && cd ..
